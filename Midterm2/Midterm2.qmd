...
1c) Using the model fit in (a), which predictor variables are statistically significant according to the individual t–tests?
...
Lecture 17: Box-Cox Transformations:
...
\rm log(Y) & , \text{if } \lambda=0
\end{cases}
$$

Transformations, previously discussed such as the square root $(\lambda=\frac{1}{2})$ and the logarithm $(\lambda=0)$ are members of the Box-Cox family. The parameter $\lambda$ is estimated by maximizing the log-likelihood function:

$$
l(\lambda)=-\frac{n}{2} \rm log(RSS(\lambda)/n)+(\lambda-1)\sum_{i=1}^nlog(y_i)
$$

where $RSS(\lambda)$ is the residuals sum of squares when $h(y_i,\lambda)$ is the response. More specifically,

$$
\rm RSS(\lambda)=\sum_{i=1}^n
[h(y_i,\lambda)-\hat \beta_0-\hat \beta_1x_{i1}-\cdots-\hat \beta_px_{ip}]^2$$

where $\hat \beta_0,\dots,\hat \beta_p$ are the least squares estimates of the parameters for transformed regression model $h(Y_i,\lambda)=\beta_0+\beta_1x_{i1}+\cdots+\beta_px_{ip}+\epsilon$
...
The scatterplot matrix and residual versus fitted plot show nonlinearity ("cuvyness") in the data that is not accounted for by the linear regression model.
...
$$
\sqrt{Y}=\beta_0+\beta_1x_1+\beta_2x_2+\beta_3x_3+\epsilon
$$
